...
本プロジェクトでは、CSSの可読性・保守性・拡張性を最優先とし、
以下のルールを厳守する。

### 1. !important の使用禁止ルール

- !important は原則使用禁止とする。
- 以下の例外のみ使用を許可する。

【使用可の例外】
- 外部ライブラリ（FontAwesome / flag-icon 等）との競合回避
- cookie-banner 等、常に最前面である必要があるUI
- 外部CSSを制御できない場合の最終手段
...
!important を追加する場合は、必ずコメントで理由を明記すること。
...
- base.css  
  → Reset / HTML・body の初期状態のみ

- layout.css  
  → レイアウト・構造（position / flex / grid / width / margin）

- theme-dark.css  
  → 色・背景・影・グラデーション・hover等の見た目

- home.css  
  → home/index.html 専用演出（hero / wave / video / animation）
...
- div p
- section * p
- タグ単体指定（p, h2, ul 等）での装飾

代わりに、意味のあるクラスを必ず付与する。
...
### 4. wrap / wrap-inner の責務

- .wrap  
  → レイアウト用コンテナ（width / margin / position のみ）

- .wrap-inner  
  → 本文・UIの世界（テキスト・コンポーネント）

.wrap に以下を記述することは禁止する。
- color
- font-size
- line-height
- margin-left
...
- 特定ページ専用の演出・変数・mask-image は
  body クラス（例：.home-page / .user-page）で必ずスコープを切る。

禁止例：
.label-attachment-wrapper { mask-image: ... }

許可例：
.user-page .label-attachment-wrapper { mask-image: ... }

---

### 6. CSS変数（:root）の運用ルール

- :root には「全ページ共通で使う変数のみ」を定義する。
- ページ専用変数は :root に置かない。
...
CSSを追加する前に、必ず以下を確認する。

- これは layout / theme / home のどれか？
- !important を使わずに解決できないか？
- wrap に依存していないか？
- 将来、別ページで副作用が出ないか？
...
本ルールは「理想論」ではなく、
本プロジェクトで実際に発生したトラブルを踏まえて策定された。

CSSは「効く」より「説明できる」ことを優先する。

---

### CSSキャッシュ対策ルール

- CSS は原則「ファイル名バージョン管理」を行う
- 内容を変更した場合は必ず新しいバージョン名を付与する
...
- クエリパラメータ（?v=xxx）は検証用にのみ使用し、
  本番では使用しない
- 古いCSSファイルは即削除せず、一定期間保管後に削除する
