...
## 15. CSS設計ガイドライン（役割と割り振り）

本ドキュメントは、AGML公式サイトにおけるCSSファイルの役割分担と、
今後の修正・追加時に守るべきルールを明確化するためのガイドです。

本プロジェクトでは、CSSの責務を明確に分離し、
「どこを触れば、どこに影響が出るか」を常に予測可能な状態に保つことを重視しています。
...
base.css は、サイト全体の「前提条件」を定義するCSSです。

**主な役割：**
- ブラウザ差異を吸収するためのリセット
- box-sizing / margin / padding の初期化
- body / html / フォント指定などの全体ルール
- 全ページで共通となる最低限の要素スタイル

**ルール：**
- ページ固有の装飾は書かない
- 色・テーマに依存する指定は極力行わない
- ここを修正すると全ページに影響が出る前提で扱う
...
layout.css は、「見た目」ではなく「構造」を定義するCSSです。

**主な役割：**
- ナビゲーション、フッター、ラッパー構造
- グリッド、flex、position などの配置制御
- コンテンツの並び順や余白の大枠
- レスポンシブ時の構造切り替え

**ルール：**
- 色・グラデーション・装飾的表現は書かない
- position / display / width / margin などに限定する
- 「どこに置くか」を決めるCSSとして扱う
...
theme-dark.css は、AGMLサイトの世界観を担うメインテーマCSSです。

**主な役割：**
- 背景色、文字色、グラデーション
- ボタン・カード・alert-box などの見た目
- ダークテーマ特有の演出
- UIコンポーネントの装飾表現

**ルール：**
- 構造（display/position）は極力触らない
- 共通UIは class ベースで定義する
- ページ固有の微調整は ID で限定的に上書きする
- alert-box などの共通コンポーネントはここに集約する
...
home.css は、特定ページ専用のCSSを置く場所です。

**主な役割：**
- トップページ特有の演出
- downloadページ、missionページなどの限定UI
- 特定ステップ、特定セクション専用の調整

**ルール：**
- 他ページに影響する指定は書かない
- 必ず body クラス（例：.download-page）でスコープを切る
- ID や限定セレクタで安全に上書きする

---

### 15.2 CSS割り振りの基本ルール（重要）

1. **共通か？固有か？を最初に判断する**
   - 共通 → base / layout / theme-dark
   - 固有 → home.css
...
3. **共通UIは class、差分は ID**
   - class：デザインシステム
   - ID：文脈ごとの微調整（例：#alert-step1）

4. **「このCSSはどこに影響するか」を常に説明できる状態にする**

---

### 15.3 具体例（alert-box）

- alert-box の基本デザイン → theme-dark.css（.alert-box）
- Step1専用の余白調整 → theme-dark.css（#alert-step1）
- HTML側では class は維持し、ID を追加して制御する
...
AGMLのCSS設計は、
「とりあえず動く」よりも
「後から触っても壊れない」ことを優先します。

このガイドに沿って修正を行うことで、
UI・思想・保守性が一貫したサイトを維持できます。

---

## 16. AGMLラベル表示における実装方針（確定）

### 16.1 基本方針（破綻防止）

- AGMLラベルの円形ゲージ表示は **SVG内部をJSで操作しない**
- `<img>` タグによる **画像切替のみ** で制御する
- 表示ロジックは JavaScript が **レベル（1〜5）を決定するだけ** に留める
- CSSは配置・サイズ指定のみとし、視覚状態（点灯／非点灯）には関与しない

---

### 16.2 ゲージ画像構成（本番）

以下の **5枚のSVGのみを本番で使用する**
...
以下のSVGは **制作素材としてのみ使用可** とし、本番では使用しない。

- agml-gauge-base.svg
- agml-gauge-5step.svg

これらをHTML / JS / CSSから参照することは禁止する。

---

### 16.4 将来のアニメーション対応について

- アニメーションは「連続値」ではなく **段階（1→2→3…）表示** を基本とする
- 実装は `src` の段階的切替で行う
- SVG内部操作やCanvas生成は **将来フェーズで再検討** する

本フェーズでは「壊れない構造」を最優先とする。