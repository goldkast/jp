...
## 追記：ファイルアップ（FTP）運用ルール（本番）

### 目的
本プロジェクト（AGML）は **GitHub Pages を使用せず**、  
**ロリポップ（FTP）による静的ファイルアップロード**を本番運用とする。

本追記は、開発・更新・反映時の混乱を防ぐための  
**運用ルール確定**を目的とする。
...
1. ローカル作業ディレクトリ  
C:\Users\goldk\Desktop\AGML\

markdown
コードをコピーする
にて編集・検証を行う。

2. 編集完了後、以下の **中身のみ** を FTP でアップロードする。  
（フォルダ構造は維持する）
...
4. 反映確認URL
https://aigenmusiclabel.com/jp/
https://aigenmusiclabel.com/app/thumbnail/
...
- GitHub は **履歴管理・バックアップ用途**として使用する
- GitHub Pages / CNAME / Custom domain は **使用しない**
- 公開の成否は **ロリポップ上の表示のみ**を基準とする

---

### 注意事項（重要）

- `public_html` 配下に **二重フォルダ（AGML/AGML 等）を作成しない**
- `index.html` の有無は **各配下ディレクトリごとに管理**する
- パス指定は **絶対パス（例：/img/xxx.png）を基本**とし、  
FTP直置き前提で設計する
...
AGML の本番環境は **ロリポップ（FTP）を唯一の公開先**とする。  
GitHub Pages に依存しない構成とすることで、
...
本追記以降、**AGML の公開・検証はすべて FTP 運用を正とする。**


#### デフォルト言語
- 現フェーズでは日本語を表示（`ja`）
- 英語対応開始時に、デフォルト言語を `en` に切り替える
- 実装は JSON 差し替えのみで対応可能

---

### i18n（国際化）設計

- 表示テキストはすべて JSON ファイルで管理
- HTML には `data-i18n` 属性のみを記述
- 画像・アイコンには原則テキストを含めない
- 翻訳対象外の固定語（例：AGML、AI-Generated、Level 等）は明示的に管理

この設計により、
- 多言語追加は JSON ファイル追加のみ
- 翻訳作業は数分で完了
- レイアウト崩れや再デザインを防止
...
- 創作と説明を混ぜない
- 誤解を生まないUIを最優先する
- 自由に見せて、事故を起こさせない制限を設ける
- 多言語化は「翻訳」ではなく「切替」として扱う

本仕様は、AGMLの思想である  
**透明性・静かな可視化・誤解のない情報提示**を維持するためのものである。


## 追記：多言語展開（i18n）設計・運用方針（全ページ共通）

### 位置づけ
AGML における多言語対応は、  
「ページを複製する方式」ではなく  
**単一ページ＋表示文言切替方式**を正規仕様とする。

本追記は、サムネイルジェネレーター以外の通常ページを含めた  
**全ページ共通の多言語設計ルール確定**を目的とする。

---

### 基本方針（重要）

- HTML 構造・JS ロジックは **全言語共通**
- 表示テキストのみを **JSON ファイルで管理**
- 言語は URL クエリ `?lang=` により切り替える
- 画像・アイコンには原則テキストを含めない
...
- 初期フェーズでは **日本語を表示**
- 英語サイト構築フェーズに移行後、  
  **デフォルト言語を英語（en）へ切り替える**
- 実装変更は JS のデフォルト値変更のみで対応可能

#### 言語判定の基本ロジック
```js
function getLang() {
  const params = new URLSearchParams(window.location.search);
  return params.get("lang") || "ja"; // 初期フェーズ
  // 英語対応開始後は "en" をデフォルトに変更
}
